...
const GameCreator = () => {
    ...
    useEffect(() => {
      const handleResize = () => {
        ...
        // Sprawdzamy, czy jest w trybie mobile landscape
        if (width > height && width < 1100) {
          setIsMobileLandscape(true);
        } else {
          setIsMobileLandscape(false); // Resetowanie, jeśli nie spełnia warunków
        }
      };
      ...
      // Wywołujemy handleResize przy pierwszym renderze, aby ustawić stan
      handleResize();
  
      // Sprzątanie: usuwamy listener przy odmontowywaniu
      return () => {
  ...
  return (
    <Flex width='100vw' flexDirection='column' height={isMobile?'100%':'100vh'}  bgSize='cover' // Użyj contain zamiast cover
     p='10px 5px' bgImage={`url(${gameStartBg})`}>
        ...
        {roomName&&<Text width='100%' textAlign='center' mt='30px' fontSize='xl'>Join code:
          <Tooltip label={tooltipText} fontSize="md"  closeOnClick={false}  >
         <Text as='span' ml={2} fontSize='lg' cursor='pointer'  color={'blue'} onClick={copyToClipboard}>{`${import.meta.env.VITE_API_BASE_INNER_URL}/game/join/${roomName}`}</Text>
         </Tooltip>
         </Text>}
        {dificulityError&&<Text mt={10} position='absolute' w='100%' top='0'  textAlign='center' fontSize='xl' color='red.600'>please select dificulity</Text>}
